...
const key = process.env.REACT_APP_API_KEY;
 ...
 async function fetchWeatherInfo() {
    try {
      const response = await axios.get(`http://dataservice.accuweather.com/locations/v1/cities/search?apikey=${key}&q=Manchester`)
  ...
  async function fetchCurrentWeather (locationKey, key){
    try {
      const response = await axios.get(`http://dataservice.accuweather.com/currentconditions/v1/${location[0].Key}?apikey=${key}`)
      const data = await response
      return data
